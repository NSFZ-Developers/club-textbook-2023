...
- `#include <iostream>` 是指包括 _\<iostream\>_ 头文件
- `int main() {...}` 是程序 _entry point_，必须返回 _int_ 类型。函数 _main_ 的内容用两个花括号括起来。
- `std::cout << "Hello world" << std::endl;` 是输出字符串 _"Hello world"_ 字符串的代码。_std_ 是命名空间。_std::cout_ 显然是指 _std_ 中的 _cout_ 函数。使用 _<<_ operator 连接字符串，可以多次使用连接多个字符串。_std::endl_ 可以**片面**认为是换行。
- `return 0` 是指 _main_ 函数返回整数 0。（与 _entry point_ ，也就是`int main()`里面的`int`对应）
