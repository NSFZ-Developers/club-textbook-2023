...
直接出现在代码中的值称为字面量（literal），如`100`、`123.45`。一般编译器会自动推导字面量的类型，我们也可以通过字面量后缀手动指定类型，常见的后缀有`l`（long）、`ul`（unsigned long）、`ll`（long long）、`ull`（unsigned long long）、`f`（float）、`lf`（double）、`llf`（long double）等。特别地，可以通过字面量前缀来指定数值类型的进制，常见的有`0x`（16进制）、`0`（8进制）、`0b`（2进制，C++14及以后）。
...
参数列表的语法与定义变量类似，多个参数中间用逗号分隔，但不能合并相同的类型（如`int x, y`）。与变量类似，参数也可以有默认值，但只能位于参数列表的末尾，这时调用函数时可以不传入这个参数所对应的实际参数。参数列表中定义的参数为形式参数，调用函数时传入的参数为实际参数。函数体中只能对形式参数进行操作，如果需要修改传入的实际参数，可以使用引用类型。
...
```
由于使用了引用类型，在函数体内部对x、y变量的操作就会反映到外部的作为实际参数传入的变量中。
